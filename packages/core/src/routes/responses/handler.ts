...
export async function handleResponses(c: Context) {
  ...
  if (!githubToken) {
    console.warn(`[${requestId}] POST /v1/responses — 401 missing auth`);
  ...
  console.log(`[${requestId}] POST /v1/responses`);
  const upstream = await createResponses(copilotToken, body);
